lt('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.00[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp1').set('linestyle', 'none');
model.result('pg19').feature('tblp1').set('linemarker', 'circle');
model.result('pg19').feature('tblp1').set('markerpos', 'datapoints');
model.result('pg19').run;
model.result('pg19').feature('tblp2').set('linemarker', 'circle');
model.result('pg19').feature('tblp2').set('linestyle', 'none');
model.result('pg19').feature('tblp2').set('markerpos', 'datapoints');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp1').set('linestyle', 'dotted');
model.result('pg19').feature('tblp1').set('linemarker', 'none');
model.result('pg19').run;
model.result('pg19').feature('tblp2').set('linemarker', 'none');
model.result('pg19').feature('tblp2').set('linestyle', 'dotted');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp1').set('linewidth', 2);
model.result('pg19').run;
model.result('pg19').feature('tblp2').set('linewidth', 2);
model.result('pg19').run;
model.result('pg19').feature('tblp3').set('linewidth', 2);
model.result('pg19').run;
model.result('pg19').feature('tblp4').set('linewidth', 2);
model.result('pg19').run;
model.result('pg19').feature('tblp5').set('linewidth', 2);
model.result('pg19').run;
model.result('pg1').run;
model.result('pg1').run;

model.param.set('M', '0.2');

model.result('pg38').run;
model.result('pg19').run;

model.param.set('M', '0.4');
model.param.set('k_i0', '2');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');
model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.result('pg1').run;
model.result('pg43').run;
model.result('pg19').run;

model.label('Electrode_model_with_paper.mph');

model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.01[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.02[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.0[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg1').run;
model.result('pg1').run;

model.param.set('M', '0.1');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.result('pg19').run;
model.result('pg41').set('window', 'window1');
model.result('pg41').run;
model.result('pg52').run;
model.result('pg48').run;
model.result.remove('pg48');
model.result.remove('pg49');
model.result.remove('pg50');
model.result.remove('pg51');
model.result.remove('pg52');
model.result('pg38').run;
model.result('pg45').run;
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [9], 0);
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [10], 0);
model.result('pg18').run;
model.result('pg39').run;
model.result('pg39').setIndex('looplevel', [9], 0);
model.result('pg39').run;
model.result('pg40').run;
model.result('pg40').set('data', 'dset9');
model.result('pg40').run;
model.result('pg40').setIndex('looplevelinput', 'manual', 0);
model.result('pg40').setIndex('looplevel', [9], 0);
model.result('pg40').run;
model.result('pg40').set('data', 'dset11');
model.result('pg40').setIndex('looplevelinput', 'manual', 2);
model.result('pg40').setIndex('looplevel', [3], 2);
model.result('pg40').setIndex('looplevel', [5], 2);
model.result('pg40').setIndex('looplevelinput', 'last', 0);
model.result('pg40').run;
model.result('pg40').setIndex('looplevelinput', 'manualindices', 0);
model.result('pg40').setIndex('looplevelindices', 5, 0);
model.result('pg40').run;
model.result('pg40').set('data', 'dset9');
model.result('pg40').setIndex('looplevelinput', 'manual', 0);
model.result('pg40').setIndex('looplevel', [2], 0);
model.result('pg40').setIndex('looplevel', [9], 0);
model.result('pg40').setIndex('looplevel', [10], 0);
model.result('pg40').run;
model.result('pg39').run;

model.study.create('std4');
model.study('std4').create('time', 'Transient');
model.study('std4').feature('time').setSolveFor('/physics/g3', true);
model.study('std4').feature('time').setSolveFor('/physics/g4', true);
model.study('std4').feature('time').setSolveFor('/physics/g', true);
model.study('std4').feature('time').setSolveFor('/physics/g2', true);
model.study('std4').feature('time').setSolveFor('/physics/dode', true);
model.study.remove('std4');
model.study('std2').feature('param').setIndex('plistarr', '9 12 15 20', 0);

model.result('pg19').run;

model.component('comp1').material('mat1').propertyGroup('ElectrodePotential').func('int1').set('extrap', 'linear');

model.result('pg19').run;
model.result('pg39').run;
model.result('pg39').set('data', 'dset11');
model.result('pg39').setIndex('looplevelinput', 'manual', 2);
model.result('pg39').setIndex('looplevel', [5], 2);
model.result('pg39').run;
model.result('pg39').setIndex('looplevelinput', 'manualindices', 0);
model.result('pg39').setIndex('looplevelindices', 'range(1,10,87)', 0);
model.result('pg39').run;
model.result('pg1').run;
model.result('pg27').run;
model.result('pg28').run;
model.result('pg23').run;
model.result('pg19').run;

model.func('int5').remove('table', 31);
model.func('int4').remove('table', 40);

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp1').active(false);
model.result('pg19').feature('tblp2').active(false);
model.result('pg19').feature('tblp3').active(false);
model.result('pg19').feature('tblp4').active(false);
model.result('pg19').feature('tblp5').active(false);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp6').active(true);
model.result('pg19').feature('tblp7').active(true);
model.result('pg19').feature('tblp8').active(true);
model.result('pg19').feature('tblp9').active(true);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp1').active(true);
model.result('pg19').feature('tblp2').active(true);
model.result('pg19').feature('tblp3').active(true);
model.result('pg19').feature('tblp4').active(true);
model.result('pg19').feature('tblp5').active(true);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.02[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp1').active(false);
model.result('pg19').feature('tblp2').active(false);
model.result('pg19').feature('tblp3').active(false);
model.result('pg19').feature('tblp4').active(false);
model.result('pg19').feature('tblp5').active(false);
model.result('pg19').run;
model.result('pg19').feature('tblp1').active(true);
model.result('pg19').run;
model.result('pg19').feature('tblp1').active(false);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;

model.sol('sol4').feature('t1').feature('st1').setIndex('stopcondarr', 'comp1.Phis_out>1.5', 0);

model.study('std2').feature('param').setIndex('plistarr', '12 4 5 7 9 12 15 20', 0);
model.study('std2').feature('param').setIndex('plistarr', '1 2 4 5 7 9 12 15 20', 0);

model.result('pg19').run;

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;
model.result('pg19').feature('tblp1').active(true);
model.result('pg19').feature('tblp2').active(true);
model.result('pg19').feature('tblp3').active(true);
model.result('pg19').feature('tblp4').active(true);
model.result('pg19').feature('tblp5').active(true);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp8').active(false);
model.result('pg19').feature('tblp9').active(false);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').setIndex('looplevelinput', 'manual', 2);
model.result('pg19').setIndex('looplevel', [1], 2);
model.result('pg19').setIndex('looplevel', [7], 2);
model.result('pg19').setIndex('looplevel', [1 2 3 4 5 6 7], 2);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.015[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.018[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;

model.func('int3').set('funcname', 'E_int');
model.func('int7').set('funcname', 'E_int1');

model.result('pg40').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.02[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg1').run;
model.result('pg20').run;
model.result('pg23').run;
model.result('pg23').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.01[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('tblp7').active(false);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').setIndex('looplevel', [1 2 3 4 5 6], 2);
model.result('pg19').run;

model.func('int6').setIndex('table', 0.3, 7, 0);
model.func('int6').setIndex('table', 0.161119331558135, 7, 1);
model.func('int6').setIndex('table', 0.2, 8, 0);
model.func('int6').setIndex('table', 0.0627391329194658, 8, 1);
model.func('int6').setIndex('table', 0.3, 7, 0);
model.func('int6').setIndex('table', 0.161119331558135, 7, 1);
model.func('int6').setIndex('table', 0.2, 8, 0);
model.func('int6').setIndex('table', 0.0627391329194658, 8, 1);

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').setIndex('looplevel', [1 2 3 4 5 6 7], 2);
model.result('pg19').run;
model.result('pg19').feature('tblp7').active(true);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').setIndex('looplevel', [1 2 3 4 5 6], 2);
model.result('pg19').run;
model.result('pg19').feature('tblp7').active(false);
model.result('pg19').run;
model.result('pg19').feature('tblp7').active(true);
model.result('pg19').run;
model.result('pg19').feature('tblp7').active(false);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg23').run;
model.result('pg2').run;
model.result('pg2').set('data', 'dset12');
model.result('pg2').set('looplevel', [85 1 6]);
model.result('pg2').run;
model.result('pg2').set('looplevel', [82 1 7]);
model.result('pg2').run;
model.result('pg2').run;
model.result('pg2').set('looplevel', [78 1 9]);
model.result('pg2').run;
model.result('pg39').run;
model.result('pg39').setIndex('looplevel', [9], 2);
model.result('pg39').setIndex('looplevel', [8], 2);
model.result('pg39').setIndex('looplevel', [9], 2);
model.result('pg39').setIndex('looplevelindices', 'range(1,10,78)', 0);
model.result('pg39').run;
model.result('pg18').run;
model.result('pg18').set('data', 'dset11');
model.result('pg18').setIndex('looplevelinput', 'manual', 2);
model.result('pg18').setIndex('looplevel', [9], 2);
model.result('pg18').setIndex('looplevelinput', 'manualindices', 0);
model.result('pg18').setIndex('looplevelindices', 'range(1,10,78)', 0);
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [7], 2);
model.result('pg18').run;
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [6], 2);
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [5], 2);
model.result('pg18').run;
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [6], 2);
model.result('pg18').run;
model.result('pg19').run;
model.result('pg19').setIndex('looplevel', [1 2 3 4 5], 2);
model.result('pg19').run;
model.result('pg19').active(false);
model.result('pg19').feature('tblp6').active(false);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').active(false);
model.result('pg19').feature('tblp1').active(false);
model.result('pg19').feature('tblp2').active(false);
model.result('pg19').feature('tblp3').active(false);
model.result('pg19').feature('tblp4').active(false);
model.result('pg19').feature('tblp5').active(false);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').active(true);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').active(true);
model.result('pg19').feature('tblp1').active(true);
model.result('pg19').feature('tblp2').active(true);
model.result('pg19').feature('tblp3').active(true);
model.result('pg19').feature('tblp4').active(true);
model.result('pg19').feature('tblp5').active(true);
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('legend', true);
model.result('pg19').feature('ptgr1').setIndex('legends', '1C', 0);
model.result('pg19').feature('ptgr1').setIndex('legends', '2C', 1);
model.result('pg19').feature('ptgr1').setIndex('legends', '4C', 2);
model.result('pg19').feature('ptgr1').setIndex('legends', '5C', 3);
model.result('pg19').feature('ptgr1').setIndex('legends', '7C', 4);
model.result('pg19').run;
model.result('pg19').set('legendpos', 'upperleft');
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.02[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.01[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg28').run;
model.result('pg27').run;
model.result('pg27').run;
model.result('pg27').run;
model.result('pg27').setIndex('looplevelinput', 'manual', 2);
model.result('pg28').run;

model.label('Electrode_model_with_paper.mph');

model.result('pg28').run;

model.component('comp2').physics.create('solid', 'SolidMechanics', 'geom3');

model.study('std1').feature('time').setSolveFor('/physics/solid', true);
model.study('std3').feature('time').setSolveFor('/physics/solid', true);
model.study('std2').feature('time').setSolveFor('/physics/solid', true);

model.component('comp2').physics.remove('solid');

model.component('comp2').variable('var2').set('FT', '((uy+1)*T_RR+(1+u_y)*T_tt*2)/(J_c^(1/3))');
model.component('comp2').variable('var2').descr('FT', '');
model.component('comp2').variable('var2').set('F_e_RR_y', '(uyy*J_c^(1/3)-(uy+1)*J_c_y)/(J_c^(2/3))');
model.component('comp2').variable('var2').descr('F_e_RR_y', '');
model.component('comp2').variable('var2').set('F_e_tt_y', '(1/y^2*(uy*y-u)*J_c^(1/3)-(u_y+1)*J_c_y)/(J_c^(2/3))');
model.component('comp2').variable('var2').descr('F_e_tt_y', '');
model.component('comp2').variable('var2').set('FT_RR_y', 'T_RR_y*F_e_RR+T_RR*F_e_RR_y');
model.component('comp2').variable('var2').descr('FT_RR_y', '');
model.component('comp2').variable('var2').set('F_e_tt', '(1+u_y)/(J_c)^(1/3)');
model.component('comp2').variable('var2').descr('F_e_tt', '');
model.component('comp2').variable('var2').set('F_e_RR', '(1+uy)/(J_c)^(1/3)');
model.component('comp2').variable('var2').descr('F_e_RR', '');
model.component('comp2').variable('var2').set('FT_tt_y', 'T_tt_y*F_e_tt+T_tt*F_e_tt_y');
model.component('comp2').variable('var2').descr('FT_tt_y', '');
model.component('comp2').variable('var2').set('FT_y', 'FT_RR_y+FT_tt_y*2');
model.component('comp2').variable('var2').descr('FT_y', '');
model.component('comp2').variable('var2').set('J_c_y', '1/3*(J_c)^(-2/3)*omega*csy');
model.component('comp2').variable('var2').descr('J_c_y', '');
model.component('comp2').variable('var2').set('FT', '((uy+1)*T_RR+(1+u_y)*T_tt*2)/(J_c^(1/3))');
model.component('comp2').variable('var2').descr('FT', '');
model.component('comp2').variable('var2').set('F_e_RR_y', '(uyy*J_c^(1/3)-(uy+1)*J_c_y)/(J_c^(2/3))');
model.component('comp2').variable('var2').descr('F_e_RR_y', '');
model.component('comp2').variable('var2').set('F_e_tt_y', '(1/y^2*(uy*y-u)*J_c^(1/3)-(u_y+1)*J_c_y)/(J_c^(2/3))');
model.component('comp2').variable('var2').descr('F_e_tt_y', '');
model.component('comp2').variable('var2').set('FT_RR_y', 'T_RR_y*F_e_RR+T_RR*F_e_RR_y');
model.component('comp2').variable('var2').descr('FT_RR_y', '');
model.component('comp2').variable('var2').set('F_e_tt', '(1+u_y)/(J_c)^(1/3)');
model.component('comp2').variable('var2').descr('F_e_tt', '');
model.component('comp2').variable('var2').set('F_e_RR', '(1+uy)/(J_c)^(1/3)');
model.component('comp2').variable('var2').descr('F_e_RR', '');
model.component('comp2').variable('var2').set('FT_tt_y', 'T_tt_y*F_e_tt+T_tt*F_e_tt_y');
model.component('comp2').variable('var2').descr('FT_tt_y', '');
model.component('comp2').variable('var2').set('FT_y', 'FT_RR_y+FT_tt_y*2');
model.component('comp2').variable('var2').descr('FT_y', '');
model.component('comp2').variable('var2').set('J_c_y', '1/3*(J_c)^(-2/3)*omega*csy');
model.component('comp2').variable('var2').descr('J_c_y', '');

model.component('comp2').physics('g4').feature('gfeq1').setIndex('Ga', {'0' '-y^2*Ds_int1(cs/cs_max_neg)*(csy-cs/R_const/T*(omega1/(J_c)^(2/3)*FT_y-2/3*omega1^2*FT*csy/(J_c)^(5/3)))'}, 0);

model.label('Electrode_model_with_paper.mph');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.sol('sol1').runAll;

model.result('pg34').run;

model.sol('sol9').study('std3');

model.study('std3').feature('time').set('notlistsolnum', 1);
model.study('std3').feature('time').set('notsolnum', 'last');
model.study('std3').feature('time').set('listsolnum', 1);
model.study('std3').feature('time').set('solnum', 'last');

model.sol('sol9').feature.remove('t1');
model.sol('sol9').feature.remove('v1');
model.sol('sol9').feature.remove('st1');
model.sol('sol9').create('st1', 'StudyStep');
model.sol('sol9').feature('st1').set('study', 'std3');
model.sol('sol9').feature('st1').set('studystep', 'time');
model.sol('sol9').create('v1', 'Variables');
model.sol('sol9').feature('v1').set('control', 'time');
model.sol('sol9').create('t1', 'Time');
model.sol('sol9').feature('t1').set('tlist', 'range(0,10,100)');
model.sol('sol9').feature('t1').set('plot', 'off');
model.sol('sol9').feature('t1').set('plotgroup', 'pg34');
model.sol('sol9').feature('t1').set('plotfreq', 'tout');
model.sol('sol9').feature('t1').set('probesel', 'all');
model.sol('sol9').feature('t1').set('probes', {'pdom1' 'dom1'});
model.sol('sol9').feature('t1').set('probefreq', 'tsteps');
model.sol('sol9').feature('t1').set('atolglobalvaluemethod', 'factor');
model.sol('sol9').feature('t1').set('endtimeinterpolation', true);
model.sol('sol9').feature('t1').set('control', 'time');
model.sol('sol9').feature('t1').create('seDef', 'Segregated');
model.sol('sol9').feature('t1').create('fc1', 'FullyCoupled');
model.sol('sol9').feature('t1').feature('fc1').set('linsolver', 'dDef');
model.sol('sol9').feature('t1').feature.remove('fcDef');
model.sol('sol9').feature('t1').feature.remove('seDef');
model.sol('sol4').feature('v1').set('notsolnum', 'auto');
model.sol('sol4').feature('v1').set('notsolvertype', 'solnum');
model.sol('sol4').feature('v1').set('notlistsolnum', {'1'});
model.sol('sol4').feature('v1').set('notsolnum', 11);
model.sol('sol4').feature('v1').set('notlistsolnum', {'1'});
model.sol('sol4').feature('v1').set('notsolnum', '11');
model.sol('sol4').feature('v1').set('control', 'time');
model.sol('sol4').feature('v1').set('solnum', 'auto');
model.sol('sol4').feature('v1').set('solvertype', 'solnum');
model.sol('sol4').feature('v1').set('listsolnum', {'1'});
model.sol('sol4').feature('v1').set('solnum', 11);
model.sol('sol4').feature('v1').set('listsolnum', {'1'});
model.sol('sol4').feature('v1').set('solnum', '11');
model.sol('sol4').feature('v1').set('control', 'time');

model.study('std2').feature('time').set('notsolnum', 'auto');
model.study('std2').feature('time').set('notsolvertype', 'solnum');
model.study('std2').feature('time').set('notlistsolnum', {'1'});
model.study('std2').feature('time').set('notsolnum', 11);
model.study('std2').feature('time').set('notlistsolnum', {'1'});
model.study('std2').feature('time').set('notsolnum', '11');
model.study('std2').feature('time').set('notsolnumhide', 'off');
model.study('std2').feature('time').set('notstudyhide', 'off');
model.study('std2').feature('time').set('notsolhide', 'on');
model.study('std2').feature('time').set('solnum', 'auto');
model.study('std2').feature('time').set('solvertype', 'solnum');
model.study('std2').feature('time').set('listsolnum', {'1'});
model.study('std2').feature('time').set('solnum', 11);
model.study('std2').feature('time').set('listsolnum', {'1'});
model.study('std2').feature('time').set('solnum', '11');
model.study('std2').feature('time').set('solnumhide', 'off');
model.study('std2').feature('time').set('initstudyhide', 'off');
model.study('std2').feature('time').set('initsolhide', 'on');

model.sol('sol9').attach('std3');

model.batch('p3').feature.remove('so1');
model.batch('p3').create('so1', 'Solutionseq');
model.batch('p3').feature('so1').set('seq', 'sol9');
model.batch('p3').feature('so1').set('store', 'on');
model.batch('p3').feature('so1').set('clear', 'on');
model.batch('p3').feature('so1').set('psol', 'sol10');
model.batch('p3').set('pname', {'C'});
model.batch('p3').set('plistarr', {'0'});
model.batch('p3').set('sweeptype', 'sparse');
model.batch('p3').set('probesel', 'all');
model.batch('p3').set('probes', {'pdom1' 'dom1'});
model.batch('p3').set('plot', 'off');
model.batch('p3').set('err', 'on');
model.batch('p3').attach('std3');
model.batch('p3').set('control', 'param');

model.result.create('pg48', 'PlotGroup2D');
model.result('pg48').set('data', 'dset16');
model.result('pg48').create('surf1', 'Surface');
model.result('pg48').feature('surf1').set('expr', 'u');
model.result.create('pg49', 'PlotGroup2D');
model.result('pg49').set('data', 'dset16');
model.result('pg49').create('surf1', 'Surface');
model.result('pg49').feature('surf1').set('expr', 'cs');
model.result.create('pg50', 'PlotGroup1D');
model.result('pg50').set('data', 'dset15');
model.result('pg50').create('lngr1', 'LineGraph');
model.result('pg50').feature('lngr1').set('xdata', 'expr');
model.result('pg50').feature('lngr1').set('xdataexpr', 'x');
model.result('pg50').feature('lngr1').selection.all;
model.result('pg50').feature('lngr1').set('expr', 'cl');
model.result.create('pg51', 'PlotGroup1D');
model.result('pg51').set('data', 'dset15');
model.result('pg51').create('lngr1', 'LineGraph');
model.result('pg51').feature('lngr1').set('xdata', 'expr');
model.result('pg51').feature('lngr1').set('xdataexpr', 'x');
model.result('pg51').feature('lngr1').selection.all;
model.result('pg51').feature('lngr1').set('expr', 'uM');
model.result.create('pg52', 'PlotGroup1D');
model.result('pg52').set('data', 'dset15');
model.result('pg52').create('lngr1', 'LineGraph');
model.result('pg52').feature('lngr1').set('xdata', 'expr');
model.result('pg52').feature('lngr1').set('xdataexpr', 'x');
model.result('pg52').feature('lngr1').selection.all;
model.result('pg52').feature('lngr1').set('expr', 'epsl_l');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.result.remove('pg52');
model.result.remove('pg50');
model.result.remove('pg51');
model.result.remove('pg49');
model.result.remove('pg48');

model.study('std2').feature('time').set('solnum', 6);
model.study('std2').feature('time').set('notsolnum', 6);

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg2').run;
model.result('pg19').run;
model.result('pg2').run;
model.result('pg2').set('looplevel', [91 1 6]);
model.result('pg2').run;
model.result('pg2').run;
model.result('pg2').set('looplevel', [85 1 9]);
model.result('pg2').run;
model.result('pg2').run;
model.result('pg19').run;

model.func('int6').setIndex('table', 0.1, 9, 0);
model.func('int6').setIndex('table', '0.0316998500247530', 9, 1);
model.func('int6').setIndex('table', 0, 10, 0);
model.func('int6').setIndex('table', 0.01, 10, 1);
model.func('int6').setIndex('table', 0.1, 9, 0);
model.func('int6').setIndex('table', '0.0316998500247530', 9, 1);
model.func('int6').setIndex('table', 0, 10, 0);
model.func('int6').setIndex('table', 0.01, 10, 1);

model.result('pg37').run;
model.result('pg37').setIndex('looplevelinput', 'manual', 2);
model.result('pg37').setIndex('looplevel', [9], 2);
model.result('pg37').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.02[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.01[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;

model.func.create('step1', 'Step');

model.result('pg19').run;

model.component('comp2').physics('g4').prop('ShapeProperty').set('shapeFunctionType', 'shlag');

model.label('Electrode_model_with_paper.mph');

model.result('pg19').run;
model.result('pg8').run;
model.result('pg8').setIndex('looplevelinput', 'manual', 2);
model.result('pg8').setIndex('looplevelinput', 'all', 2);
model.result('pg1').run;

model.func.remove('step1');

model.result('pg2').run;
model.result('pg2').set('looplevel', [85 1 6]);
model.result('pg2').run;
model.result('pg2').set('looplevel', [89 1 6]);
model.result('pg2').run;
model.result('pg2').set('looplevel', [90 1 6]);
model.result('pg2').run;
model.result('pg2').set('looplevel', [91 1 6]);
model.result('pg2').run;
model.result('pg19').run;
model.result('pg25').run;
model.result('pg30').run;
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [5], 2);
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [1 2 3 4 5], 2);
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [5], 2);
model.result('pg18').run;

model.study('std2').feature('param').setIndex('plistarr', '1 2 4 5 7', 0);

model.func('int1').set('extrap', 'const');
model.func.remove('int3');
model.func.remove('int2');

model.param.set('k_i0', '1');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg25').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.02[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.01[V]/I_input_1C)');
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;

model.component('comp1').variable('var3').set('j_react_micro', 'i0*(exp(aa_neg*eta*F_const/R_const/T)*exp(-aa_neg*M*T_h_all*omega/J_c_micro^(2/3)/R_const/T)-exp(-ac_neg*eta*F_const/R_const/T)*exp(ac_neg*M*T_h_all*omega/J_c_micro^(2/3)/R_const/T))', 'reaction current for reference configuration in macroscale');
model.component('comp1').variable('var3').set('j_react_micro', 'i0*(exp(aa_neg*eta*F_const/R_const/T)*exp(-aa_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T)-exp(-ac_neg*eta*F_const/R_const/T)*exp(ac_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T))');
model.component('comp1').variable('var3').descr('j_react_micro', 'reaction current for reference configuration in macroscale');
model.component('comp1').variable('var3').remove('rp_primary_1');
model.component('comp1').variable('var3').remove('rp_primary_1_M');
model.component('comp1').variable('var3').remove('sigma_h_micro');
model.component('comp1').variable('var3').remove('T_h_all');
model.component('comp1').variable('var3').rename('T_h_micro', 'sigma_h_micro');
model.component('comp1').variable('var3').set('sigma_h_micro', 'comp2.linext1(comp2.sigma_h)');
model.component('comp1').variable('var3').descr('sigma_h_micro', '');
model.component('comp1').variable('var3').set('rp_primary_1', 'rp_primary+comp2.linext1(comp2.u)');
model.component('comp1').variable('var3').descr('rp_primary_1', 'current configuration in microscale');
model.component('comp1').variable('var3').set('rp_primary_1_M', 'rp_primary_1/(det_F_M)^(1/3)');
model.component('comp1').variable('var3').descr('rp_primary_1_M', 'reference configuration in macroscale');
model.component('comp1').variable('var3').set('FT_micro', 'comp2.linext1(comp2.FT)');
model.component('comp1').variable('var3').descr('FT_micro', '');
model.component('comp1').variable('var3').set('Tr_Fe_micro', 'comp2.linext1(comp2.F_e_RR+2*comp2.F_e_tt)');
model.component('comp1').variable('var3').descr('Tr_Fe_micro', '');
model.component('comp1').variable('var3').set('FT_macro', 'Tr_Fe_micro*T_h_M/epsl_s');
model.component('comp1').variable('var3').descr('FT_macro', '');
model.component('comp1').variable('var3').set('FT', 'FT_macro+FT_micro');
model.component('comp1').variable('var3').descr('FT', '');
model.component('comp1').variable('var3').clear;
model.component('comp1').variable('var3').set('j_react_micro', 'i0*(exp(aa_neg*eta*F_const/R_const/T)*exp(-aa_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T)-exp(-ac_neg*eta*F_const/R_const/T)*exp(ac_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T))');
model.component('comp1').variable('var3').descr('j_react_micro', 'reaction current for reference configuration in macroscale');
model.component('comp1').variable('var3').set('eta', '(phis[V]-phil[V]-mat1.elpot.Eeq_int1(cs_surf/cs_max_neg))');
model.component('comp1').variable('var3').descr('eta', '');
model.component('comp1').variable('var3').set('i0', 'max(eps^2,k_i0*i0_int1(cs_surf/cs_max_neg)*(max(cl[mol/m^3]/cl_ref,eps^2))^aa_neg)');
model.component('comp1').variable('var3').descr('i0', '');
model.component('comp1').variable('var3').set('cs_surf', 'comp2.linext1(comp2.cs)');
model.component('comp1').variable('var3').descr('cs_surf', '');
model.component('comp1').variable('var3').set('cs_ave', 'comp2.linproj1(comp2.cs*y^2)/rp_primary^3*3');
model.component('comp1').variable('var3').descr('cs_ave', '');
model.component('comp1').variable('var3').set('sigma_h_micro', 'comp2.linext1(comp2.sigma_h)');
model.component('comp1').variable('var3').descr('sigma_h_micro', '');
model.component('comp1').variable('var3').set('a_v_0', '3*epsl_s*rp_primary^2/rp_primary_1^3*det_F_M');
model.component('comp1').variable('var3').descr('a_v_0', '');
model.component('comp1').variable('var3').set('j_react_macro', 'j_react_micro');
model.component('comp1').variable('var3').descr('j_react_macro', 'reaction current for reference configuration in microscale');
model.component('comp1').variable('var3').set('rp_primary_1', 'rp_primary+comp2.linext1(comp2.u)');
model.component('comp1').variable('var3').descr('rp_primary_1', 'current configuration in microscale');
model.component('comp1').variable('var3').set('rp_primary_1_M', 'rp_primary_1/(det_F_M)^(1/3)');
model.component('comp1').variable('var3').descr('rp_primary_1_M', 'reference configuration in macroscale');
model.component('comp1').variable('var3').set('FT_micro', 'comp2.linext1(comp2.FT)');
model.component('comp1').variable('var3').descr('FT_micro', '');
model.component('comp1').variable('var3').set('J_c_micro', 'comp2.linext1(comp2.J_c)');
model.component('comp1').variable('var3').descr('J_c_micro', '');
model.component('comp1').variable('var3').set('epsl_l_1', 'max(epsl_l,eps^2)');
model.component('comp1').variable('var3').descr('epsl_l_1', '');
model.component('comp1').variable('var3').set('a', '3/rp_primary*epsl_s_0');
model.component('comp1').variable('var3').descr('a', '');
model.component('comp1').variable('var3').set('b', '1');
model.component('comp1').variable('var3').descr('b', '');
model.component('comp1').variable('var3').set('a_v', 'a_v_0');
model.component('comp1').variable('var3').descr('a_v', '');
model.component('comp1').variable('var3').set('epsl_l_2', '(1-0.21-((cs_ave-cs_0_neg)*omega+1)*epsl_s_0/det_F_M)');
model.component('comp1').variable('var3').descr('epsl_l_2', '');
model.component('comp1').variable('var3').set('Tr_Fe_micro', 'comp2.linext1(comp2.F_e_RR+2*comp2.F_e_tt)');
model.component('comp1').variable('var3').descr('Tr_Fe_micro', '');
model.component('comp1').variable('var3').set('FT_macro', 'Tr_Fe_micro*T_h_M/epsl_s');
model.component('comp1').variable('var3').descr('FT_macro', '');
model.component('comp1').variable('var3').set('FT', 'FT_macro+FT_micro');
model.component('comp1').variable('var3').descr('FT', '');
model.component('comp1').variable('var3').set('j_react_micro', 'i0*(exp(aa_neg*eta*F_const/R_const/T)*exp(-aa_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T)-exp(-ac_neg*eta*F_const/R_const/T)*exp(ac_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T))', 'reaction current for reference configuration in macroscale');
model.component('comp1').variable('var3').set('j_react_micro', 'i0*(exp(aa_neg*eta*F_const/R_const/T)*exp(-aa_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T)-exp(-ac_neg*eta*F_const/R_const/T)*exp(ac_neg*M*FT*omega/J_c_micro^(2/3)/R_const/T))');
model.component('comp1').variable('var3').descr('j_react_micro', 'reaction current for reference configuration in macroscale');
model.component('comp1').variable('var3').set('eta', '(phis[V]-phil[V]-mat1.elpot.Eeq_int1(cs_surf/cs_max_neg))');
model.component('comp1').variable('var3').descr('eta', '');
model.component('comp1').variable('var3').set('i0', 'max(eps^2,k_i0*i0_int1(cs_surf/cs_max_neg)*(max(cl[mol/m^3]/cl_ref,eps^2))^aa_neg)');
model.component('comp1').variable('var3').descr('i0', '');
model.component('comp1').variable('var3').set('cs_surf', 'comp2.linext1(comp2.cs)');
model.component('comp1').variable('var3').descr('cs_surf', '');
model.component('comp1').variable('var3').set('cs_ave', 'comp2.linproj1(comp2.cs*y^2)/rp_primary^3*3');
model.component('comp1').variable('var3').descr('cs_ave', '');
model.component('comp1').variable('var3').set('sigma_h_micro', 'comp2.linext1(comp2.sigma_h)');
model.component('comp1').variable('var3').descr('sigma_h_micro', '');
model.component('comp1').variable('var3').set('a_v_0', '3*epsl_s*rp_primary^2/rp_primary_1^3*det_F_M');
model.component('comp1').variable('var3').descr('a_v_0', '');
model.component('comp1').variable('var3').set('j_react_macro', 'j_react_micro', 'reaction current for reference configuration in microscale');
model.component('comp1').variable('var3').set('j_react_macro', 'j_react_micro');
model.component('comp1').variable('var3').descr('j_react_macro', 'reaction current for reference configuration in microscale');
model.component('comp1').variable('var3').set('rp_primary_1', 'rp_primary+comp2.linext1(comp2.u)', 'current configuration in microscale');
model.component('comp1').variable('var3').set('rp_primary_1', 'rp_primary+comp2.linext1(comp2.u)');
model.component('comp1').variable('var3').descr('rp_primary_1', 'current configuration in microscale');
model.component('comp1').variable('var3').set('rp_primary_1_M', 'rp_primary_1/(det_F_M)^(1/3)', 'reference configuration in macroscale');
model.component('comp1').variable('var3').set('rp_primary_1_M', 'rp_primary_1/(det_F_M)^(1/3)');
model.component('comp1').variable('var3').descr('rp_primary_1_M', 'reference configuration in macroscale');
model.component('comp1').variable('var3').set('FT_micro', 'comp2.linext1(comp2.FT)');
model.component('comp1').variable('var3').descr('FT_micro', '');
model.component('comp1').variable('var3').set('J_c_micro', 'comp2.linext1(comp2.J_c)');
model.component('comp1').variable('var3').descr('J_c_micro', '');
model.component('comp1').variable('var3').set('epsl_l_1', 'max(epsl_l,eps^2)');
model.component('comp1').variable('var3').descr('epsl_l_1', '');
model.component('comp1').variable('var3').set('a', '3/rp_primary*epsl_s_0');
model.component('comp1').variable('var3').descr('a', '');
model.component('comp1').variable('var3').set('b', '1');
model.component('comp1').variable('var3').descr('b', '');
model.component('comp1').variable('var3').set('a_v', 'a_v_0');
model.component('comp1').variable('var3').descr('a_v', '');
model.component('comp1').variable('var3').set('epsl_l_2', '(1-0.21-((cs_ave-cs_0_neg)*omega+1)*epsl_s_0/det_F_M)');
model.component('comp1').variable('var3').descr('epsl_l_2', '');
model.component('comp1').variable('var3').set('Tr_Fe_micro', 'comp2.linext1(comp2.F_e_RR+2*comp2.F_e_tt)');
model.component('comp1').variable('var3').descr('Tr_Fe_micro', '');
model.component('comp1').variable('var3').set('FT_macro', 'Tr_Fe_micro*T_h_M/epsl_s');
model.component('comp1').variable('var3').descr('FT_macro', '');
model.component('comp1').variable('var3').set('FT', 'FT_macro+FT_micro');
model.component('comp1').variable('var3').descr('FT', '');

model.label('Electrode_model_with_paper.mph');

model.component('comp1').variable('var3').set('FT', 'FT_micro');

model.component('comp2').physics('g4').feature('gfeq1').setIndex('Ga', {'0' '-y^2*Ds_int1(cs/cs_max_neg)*(csy-cs/R_const/T*(omega1/(J_c)^(2/3)*FT_y/3-2/3*omega1^2*FT/3*csy/(J_c)^(5/3)))'}, 0);

model.component('comp1').variable('var3').set('j_react_micro', 'i0*(exp(aa_neg*eta*F_const/R_const/T)*exp(-aa_neg*M*FT/3*omega/J_c_micro^(2/3)/R_const/T)-exp(-ac_neg*eta*F_const/R_const/T)*exp(ac_neg*M*FT/3*omega/J_c_micro^(2/3)/R_const/T))');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').feature('ptgr1').set('expr', 'ppb1[V]+(I_input-I_input_1C)*(0.00[V]/I_input_1C)');
model.result('pg19').run;

model.param.set('k_i0', '2');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg20').run;
model.result('pg1').run;
model.result('pg1').run;
model.result('pg1').feature('ptgr1').set('expr', '-M*FT/3*omega/J_c_micro^(2/3)/F_const');
model.result('pg1').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg19').run;
model.result('pg1').run;

model.func.duplicate('int8', 'int6');
model.func('int6').remove('table', [7 8 9 10]);

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;
model.result('pg2').run;
model.result('pg2').set('looplevel', [88 1 5]);
model.result('pg2').run;
model.result('pg19').run;

model.func('int6').setIndex('table', 0.3, 7, 0);
model.func('int6').setIndex('table', 0.161119331558135, 7, 1);
model.func('int6').setIndex('table', 0.3, 7, 0);
model.func('int6').setIndex('table', 0.161119331558135, 7, 1);

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;

model.label('Electrode_model_with_paper.mph');

model.result('pg19').run;
model.result('pg19').setIndex('looplevel', [3], 2);
model.result('pg19').setIndex('looplevelinput', 'all', 2);

model.component('comp1').variable('var4').remove('E_macro');
model.component('comp1').variable('var4').set('E_macro', 'E_b*1.52*epsl_s', 'E_int1(cs_ave/cs_max_neg)*epsl_s');
model.component('comp1').variable('var4').set('E_b', '0.8[GPa]');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;

model.label('Electrode_model_with_paper.mph');

model.result('pg19').run;
model.result('pg8').run;
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [4], 2);
model.result('pg18').run;
model.result('pg18').setIndex('looplevel', [5], 2);
model.result('pg18').run;
model.result('pg19').run;

model.component('comp1').variable('var4').set('kl_eff', 'k_kl*kl_int1(cl)*epsl_l_1^1.5');

model.param.set('k_kl', '2');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;

model.param.set('k_i0', '30');

model.study('std2').feature('param').setIndex('plistarr', '1 2', 0);

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;

model.param.set('k_i0', '10');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;

model.study('std2').feature('param').setIndex('plistarr', '1 2 4 5 7', 0);

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;

model.param.set('k_kl', '1');

model.result('pg1').run;
model.result('pg1').run;
model.result('pg19').run;

model.param.set('k_i0', '5');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p2').run('compute');

model.result('pg1').run;
model.result('pg19').run;
model.result('pg1').run;
model.result('pg1').run;
model.result('pg1').run;
model.result('pg1').run;
model.result('pg1').run;
model.result('pg27').run;
model.result('pg27').setIndex('looplevel', [3], 2);
model.result('pg27').run;
model.result('pg27').setIndex('looplevel', [4], 2);
model.result('pg27').run;
model.result('pg27').setIndex('looplevel', [5], 2);
model.result('pg27').run;
model.result('pg27').setIndex('looplevel', [4], 2);
model.result('pg27').run;
model.result('pg27').setIndex('looplevel', [3], 2);
model.result('pg27').run;
model.result('pg26').run;
model.result('pg27').run;
model.result('pg27').setIndex('looplevel', [4], 2);
model.result('pg27').run;
model.result('pg27').run;
model.result('pg27').feature('lngr1').set('expr', 'comp2.linext1(comp2.T_h)');
model.result('pg27').run;
model.result('pg27').feature('lngr1').set('expr', 'comp2.linext1(comp2.FT)');
model.result('pg27').run;
model.result('pg27').run;
model.result('pg27').setIndex('looplevel', [1], 2);
model.result('pg27').run;

model.param.set('rp_primary', '1.72[um]');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.sol('sol1').runAll;

model.result('pg34').run;

model.sol('sol9').study('std3');

model.study('std3').feature('time').set('notlistsolnum', 1);
model.study('std3').feature('time').set('notsolnum', 'last');
model.study('std3').feature('time').set('listsolnum', 1);
model.study('std3').feature('time').set('solnum', 'last');

model.sol('sol9').feature.remove('t1');
model.sol('sol9').feature.remove('v1');
model.sol('sol9').feature.remove('st1');
model.sol('sol9').create('st1', 'StudyStep');
model.sol('sol9').feature('st1').set('study', 'std3');
model.sol('sol9').feature('st1').set('studystep', 'time');
model.sol('sol9').create('v1', 'Variables');
model.sol('sol9').feature('v1').set('control', 'time');
model.sol('sol9').create('t1', 'Time');
model.sol('sol9').feature('t1').set('tlist', 'range(0,10,100)');
model.sol('sol9').feature('t1').set('plot', 'off');
model.sol('sol9').feature('t1').set('plotgroup', 'pg34');
model.sol('sol9').feature('t1').set('plotfreq', 'tout');
model.sol('sol9').feature('t1').set('probesel', 'all');
model.sol('sol9').feature('t1').set('probes', {'pdom1' 'dom1'});
model.sol('sol9').feature('t1').set('probefreq', 'tsteps');
model.sol('sol9').feature('t1').set('atolglobalvaluemethod', 'factor');
model.sol('sol9').feature('t1').set('endtimeinterpolation', true);
model.sol('sol9').feature('t1').set('control', 'time');
model.sol('sol9').feature('t1').create('seDef', 'Segregated');
model.sol('sol9').feature('t1').create('fc1', 'FullyCoupled');
model.sol('sol9').feature('t1').feature('fc1').set('linsolver', 'dDef');
model.sol('sol9').feature('t1').feature.remove('fcDef');
model.sol('sol9').feature('t1').feature.remove('seDef');
model.sol('sol4').feature('v1').set('notsolnum', 'auto');
model.sol('sol4').feature('v1').set('notsolvertype', 'solnum');
model.sol('sol4').feature('v1').set('notlistsolnum', {'1'});
model.sol('sol4').feature('v1').set('notsolnum', 6);
model.sol('sol4').feature('v1').set('notlistsolnum', {'1'});
model.sol('sol4').feature('v1').set('notsolnum', '6');
model.sol('sol4').feature('v1').set('control', 'time');
model.sol('sol4').feature('v1').set('solnum', 'auto');
model.sol('sol4').feature('v1').set('solvertype', 'solnum');
model.sol('sol4').feature('v1').set('listsolnum', {'1'});
model.sol('sol4').feature('v1').set('solnum', 6);
model.sol('sol4').feature('v1').set('listsolnum', {'1'});
model.sol('sol4').feature('v1').set('solnum', '6');
model.sol('sol4').feature('v1').set('control', 'time');

model.study('std2').feature('time').set('notsolnum', 'auto');
model.study('std2').feature('time').set('notsolvertype', 'solnum');
model.study('std2').feature('time').set('notlistsolnum', {'1'});
model.study('std2').feature('time').set('notsolnum', 6);
model.study('std2').feature('time').set('notlistsolnum', {'1'});
model.study('std2').feature('time').set('notsolnum', '6');
model.study('std2').feature('time').set('notsolnumhide', 'off');
model.study('std2').feature('time').set('notstudyhide', 'off');
model.study('std2').feature('time').set('notsolhide', 'on');
model.study('std2').feature('time').set('solnum', 'auto');
model.study('std2').feature('time').set('solvertype', 'solnum');
model.study('std2').feature('time').set('listsolnum', {'1'});
model.study('std2').feature('time').set('solnum', 6);
model.study('std2').feature('time').set('listsolnum', {'1'});
model.study('std2').feature('time').set('solnum', '6');
model.study('std2').feature('time').set('solnumhide', 'off');
model.study('std2').feature('time').set('initstudyhide', 'off');
model.study('std2').feature('time').set('initsolhide', 'on');

model.sol('sol9').attach('std3');

model.batch('p3').feature.remove('so1');
model.batch('p3').create('so1', 'Solutionseq');
model.batch('p3').feature('so1').set('seq', 'sol9');
model.batch('p3').feature('so1').set('store', 'on');
model.batch('p3').feature('so1').set('clear', 'on');
model.batch('p3').feature('so1').set('psol', 'sol10');
model.batch('p3').set('pname', {'C'});
model.batch('p3').set('plistarr', {'0'});
model.batch('p3').set('sweeptype', 'sparse');
model.batch('p3').set('probesel', 'all');
model.batch('p3').set('probes', {'pdom1' 'dom1'});
model.batch('p3').set('plot', 'off');
model.batch('p3').set('err', 'on');
model.batch('p3').attach('std3');
model.batch('p3').set('control', 'param');

model.result.create('pg48', 'PlotGroup2D');
model.result('pg48').set('data', 'dset16');
model.result('pg48').create('surf1', 'Surface');
model.result('pg48').feature('surf1').set('expr', 'u');
model.result.create('pg49', 'PlotGroup2D');
model.result('pg49').set('data', 'dset16');
model.result('pg49').create('surf1', 'Surface');
model.result('pg49').feature('surf1').set('expr', 'cs');
model.result.create('pg50', 'PlotGroup1D');
model.result('pg50').set('data', 'dset15');
model.result('pg50').create('lngr1', 'LineGraph');
model.result('pg50').feature('lngr1').set('xdata', 'expr');
model.result('pg50').feature('lngr1').set('xdataexpr', 'x');
model.result('pg50').feature('lngr1').selection.all;
model.result('pg50').feature('lngr1').set('expr', 'cl');
model.result.create('pg51', 'PlotGroup1D');
model.result('pg51').set('data', 'dset15');
model.result('pg51').create('lngr1', 'LineGraph');
model.result('pg51').feature('lngr1').set('xdata', 'expr');
model.result('pg51').feature('lngr1').set('xdataexpr', 'x');
model.result('pg51').feature('lngr1').selection.all;
model.result('pg51').feature('lngr1').set('expr', 'uM');
model.result.create('pg52', 'PlotGroup1D');
model.result('pg52').set('data', 'dset15');
model.result('pg52').create('lngr1', 'LineGraph');
model.result('pg52').feature('lngr1').set('xdata', 'expr');
model.result('pg52').feature('lngr1').set('xdataexpr', 'x');
model.result('pg52').feature('lngr1').selection.all;
model.result('pg52').feature('lngr1').set('expr', 'epsl_l');

model.component('comp1').probe('pdom1').genResult('none');
model.component('comp1').probe('dom1').genResult('none');

model.batch('p3').run('compute');

mod